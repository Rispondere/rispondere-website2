...
3. `google-apps-script.js` ファイルの内容をすべてコピー
4. Apps Script エディタに貼り付け（既存コードを全て削除してから貼り付け）
5. 保存ボタン（💾）をクリック
6. 関数選択で「updateRispondereSettings」を選択
7. 実行ボタン（▶）をクリック
8. 初回実行時は権限の許可が必要です（「権限を確認」→ アカウント選択 → 「許可」）
9. 完了！ポップアップで更新完了のメッセージが表示されます

**特徴:**
- ✅ ワンクリックで自動更新
- ✅ ヘッダー行に自動で色とスタイル適用
- ✅ 枠線と列幅を自動調整
- ✅ カスタムメニューが追加される（「🔄 Rispondere更新」）
